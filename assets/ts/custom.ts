...
const profileFootprintPoint: FootprintPoint = {
    title: '现居地',
    url: '/about/',
    place: '深圳',
    lat: 22.5431,
...
async function initFootprintWidget(widget: HTMLElement) {
    ...
    const markerPoints = [...points, profileFootprintPoint];

    if (!svg || !globe || !empty || !tip || !zoomLabel || !zoomInButton || !zoomOutButton || !resetButton) return;
    empty.hidden = markerPoints.length > 0;
